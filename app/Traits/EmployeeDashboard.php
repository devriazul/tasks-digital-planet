...
trait EmployeeDashboard
{
    ...
    public function employeeDashboard()
    {
        ...
        $day = carbon::now()->addMonth()->format('m-d');
       
        $this->birthday = User::select('users.*')->with(['employeeDetail', 'employeeDetail.designation'])->join('employee_details', 'employee_details.user_id', 'users.id')
            ->whereRaw('DATE_FORMAT(employee_details.`date_of_birth`, "%m-%d") BETWEEN "'.$currentDay.'" AND "'.$day.'"')
            ->get();
